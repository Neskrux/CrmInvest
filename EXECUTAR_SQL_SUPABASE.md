...
## 2. Depois execute este para corrigir o comentário:

```sql
-- Corrigir comentário que estava na tabela errada
COMMENT ON COLUMN solicitacoes_carteira.etapa_aprovacao IS 'Controla em qual etapa a aprovação está: aguardando contratos, contratos enviados ou contratos aprovados';
```

## 3. Verificar se o bucket 'documentos' existe no Supabase Storage

Vá em Storage > Buckets e crie o bucket 'documentos' se não existir.

## 4. Testar o upload

Após executar o SQL, teste novamente o upload de contratos.
