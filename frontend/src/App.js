...
import CobrancaClinica from './components/CobrancaClinica';
import SolicitacoesCobrancaAdmin from './components/SolicitacoesCobrancaAdmin';
...
const GestaoBoletosAdmin = React.lazy(() => import('./components/GestaoBoletosAdmin'));
  ...
  const RenderContent = () => {
    ...
    if (user?.tipo === 'clinica') {
      return (
        <Routes>
          ...
          />
          {/* Rota de Cobrança apenas para clínicas da empresa_id 3 */}
          {user?.empresa_id === 3 && (
            <Route path="/cobranca" element={<CobrancaClinica />} />
          )}
    ...
    return (
      <Routes>
        ...
        />
        {/* Rota de Solicitações de Cobrança - Apenas para admin da empresa_id 3 */}
        {user?.tipo === 'admin' && user?.empresa_id === 3 && (
          <Route path="/solicitacoes-cobranca" element={<SolicitacoesCobrancaAdmin />} />
        )}
        <Route path="/perfil" element={<Perfil />} />
        <Route path="/" element={<Navigate to="/dashboard" replace />} />
        <Route path="*" element={<Navigate to="/dashboard" replace />} />
    ...
    <div className="App">
      ...
      >
        ...
        <nav className="sidebar-nav">
          ...
          {/* Cobrança - Apenas para clínicas da empresa_id 3 */}
          {user.tipo === 'clinica' && user.empresa_id === 3 && (
            <div className="nav-item">
              <Link
                to="/cobranca"
                className={`nav-link ${activeTab === 'cobranca' ? 'active' : ''}`}
                onClick={handleMobileNavigation}
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {/* Solicitações de Cobrança - Apenas para admin da empresa_id 3 */}
          {user.tipo === 'admin' && user.empresa_id === 3 && (
            <div className="nav-item">
              <Link
                to="/solicitacoes-cobranca"
                className={`nav-link ${activeTab === 'solicitacoes-cobranca' ? 'active' : ''}`}
                onClick={handleMobileNavigation}
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
          ...
          {/* Gerenciar Assinatura - Apenas Admin */}
          {isAdmin && (
            <div className="nav-item">
              <Link
                to="/gerenciar-assinatura"
                className={`nav-link ${activeTab === 'gerenciar-assinatura' ? 'active' : ''}`}
                onClick={handleMobileNavigation}
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M12 20h9"/>
          ...
          {(user.tipo === 'admin' || user.tipo === 'clinica') && (
            <div className="nav-item">
              <Link
                to="/gestao-boletos"
                className={`nav-link ${activeTab === 'gestao-boletos' ? 'active' : ''}`}
                onClick={handleMobileNavigation}
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
